...
contract InvBaseData is AdminRole {
    ...
    function getBindAccBatch(address[] memory _accList)
        public view
        returns (address[] memory _vList)
    {
        _vList = new address[](_accList.length);
        for (uint256 i=0;i<_accList.length;i++){
            _vList[i] = bindDict[_accList[i]];
        }
    }
